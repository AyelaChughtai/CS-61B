...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
                              int targetYear) {
        int power = numYears(targetYear);
        double future_val = presentValue * (Math.pow((1 + (rate / 100)), power));
        return future_val;
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        int power = numYears(targetYear);
        double future_val = futureValue(presentValue, rate, targetYear);
        double future_val_real = future_val * Math.pow((1 - (inflationRate / 100)), power);
        return future_val_real;
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        double new_rate = (1 + (rate / 100));
        double savings = perYear;
        int year = targetYear;
        while (year > THIS_YEAR) {
            savings += futureValue(perYear, rate, year);
            year -= 1;
        }
        return savings;
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        int power = numYears(targetYear);
        double savings_initial = totalSavings(perYear, targetYear, rate);
        double savings_real = savings_initial * Math.pow((1 - (inflationRate / 100)), power);
        return savings_real;
    }
