...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        for (int i = 0; i < m.length; i++) {
            for (int j = 0; j < m[0].length; j++) {
                ...
                double minE = 0;
                minE = Math.min(get(am, i - 1, j - 1), get(am, i - 1, j + 1));
                if (minE > get(am, i - 1, j)) {
                    minE = get(am, i - 1, j);
                }
                am[i][j] = minE + m[i][j];
            }
    ...
    public static double get(double[][] e, int r, int c) {
        if (r < e.length && c < e[0].length && r >= 0 && c >= 0) {
            return e[r][c];
        } else {
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) { return null;}
