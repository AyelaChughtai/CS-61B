...
class Game {
     ...
     * @param line string.
     */
    private void processCommandHelper(String line) {
        line = line.trim();
        if (line.length() == 0) {
            return;
        }
    ...
    /** The auto white case.
     */
    private void autoWhite() {
        if (_white.isManual()) {
            autoCommand("white");
        }
    }

    /** The manual black case.
     */
    private void manualBlackConvert() {
        if (!_black.isManual()) {
            manualCommand("black");
        }
    }

    /** The manual white case.
     */
    private void manualWhiteConvert() {
        if (!_white.isManual()) {
            manualCommand("white");
        }
    }

    /** The rest of the undo case.
     */
    private void undo() {
        if ((manualBlack() && manualWhite())
                || (!manualBlack() && !manualWhite())) {
            _board.retract();
        } else {
            _board.retract();
            _board.retract();
