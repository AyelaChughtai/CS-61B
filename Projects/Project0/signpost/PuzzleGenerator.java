...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        int connectableSquares = 0;
        int seqNumSt = start.sequenceNum();
        int last = model.width() * model.height();
        Sq returnSquare = null;
        if (seqNumSt == last) {
            return null;
        }
        for (Sq square : model) {
            if (start.connectable(square)) {
                int seqNumSq = square.sequenceNum();
                if (seqNumSt > 0) {
                    if (seqNumSq == seqNumSt + 1
                            || (seqNumSq == 0
                            &&
                            !model.solnNumToSq(seqNumSt + 1).hasFixedNum())) {
                        returnSquare = square;
                        connectableSquares += 1;
                    }
                } else if ((seqNumSq != 1) && (seqNumSq == 0
                        || !model.solnNumToSq(seqNumSq - 1).hasFixedNum())) {
                    returnSquare = square;
                    connectableSquares += 1;
                }
            }
        }
        if (connectableSquares == 1) {
            return returnSquare;
        }
        return null;
    }
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        ...
        for (Sq square : model) {
            if (square.connectable(end)) {
                int seqNumSq = square.sequenceNum();
                if (seqNumEnd > 1) {
                    if (end.sequenceNum() == seqNumEnd - 1) {
                        returnSquare = square;
                        connectableSquares += 1;
                    }
                } else if ((seqNumSq != last) && (seqNumSq == 0
                        || !model.solnNumToSq(seqNumSq + 1).hasFixedNum())) {
                    returnSquare = square;
                    connectableSquares += 1;
                }
